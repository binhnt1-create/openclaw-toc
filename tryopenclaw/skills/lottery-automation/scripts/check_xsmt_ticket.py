...
def main():
    ...
    ap.add_argument(
        "--date-token",
        metavar="DDMMYYYY",
        help='Build section id as mt_kqngay_<token> (use this or --section-id)',
    )
    ap.add_argument(
        "--section-id",
        help="Override section id (e.g. mt_kqngay_17042026). If omitted, --date-token is required.",
    )
    ...
    if not section_id:
        if not args.date_token:
            raise SystemExit("Provide --date-token DDMMYYYY or --section-id mt_kqngay_...")
        section_id = f"mt_kqngay_{args.date_token}"
