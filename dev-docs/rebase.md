...
Chiến lược **chained rebase**: mỗi version checkout từ kết quả version trước.
Squash chỉ thực hiện **1 lần duy nhất** ở version đầu tiên.

```
tuandh/poc_custom_openclaw (nhiều commits)
    ↓ squash + rebase --onto v2026.4.20 origin/main
rebase-vs-v2026.4.20  (1 squashed commit trên v2026.4.20)
    ↓ rebase --onto v2026.4.21 v2026.4.20
rebase-vs-v2026.4.21  (1 squashed commit trên v2026.4.21)
    ↓ ...
rebase-vs-v2026.5.5
```
...
### Lần đầu — từ POC branch gốc

#### Bước 0 — Release notes & file phân tích
...
Dán output vào AI để phân tích risk, tạo `dev-docs/rebase/$VERSION.md`.

#### Bước 1 — Checkout từ POC branch

```bash
git checkout -b rebase-vs-$VERSION tuandh/poc_custom_openclaw
```

#### Bước 2 — Squash tất cả POC commits thành 1

```bash
git rebase -i origin/main
# Đổi tất cả dòng trừ dòng đầu thành "s" (squash)
# Commit message gọn: "feat(toc): skill mention layer — squashed"
```
...
#### Bước 3 — Rebase lên upstream tag

```bash
git fetch upstream --tags
git rebase --onto $VERSION origin/main
# Nếu conflict: fix → git add → git rebase --continue (chỉ 1 lần)
```

---

### Các version kế tiếp — chained từ version trước

Không cần squash lại — đã có sẵn 1 commit squashed.
...
PREV=v2026.4.20
gh release view $VERSION --repo openclaw/openclaw
# Tạo dev-docs/rebase/$VERSION.md
```

#### Bước 1 — Checkout từ version trước

```bash
git checkout -b rebase-vs-$VERSION rebase-vs-$PREV
```

#### Bước 2 — Rebase lên upstream tag mới

```bash
git fetch upstream --tags
git rebase --onto $VERSION $PREV
# Chỉ replay delta giữa 2 version → ít conflict hơn
# Nếu conflict: fix → git add → git rebase --continue
```

---

### Bước cuối — Verify & push (áp dụng cho cả 2 trường hợp)

```bash
pnpm install     # nếu package.json thay đổi
pnpm check       # lint + typecheck
pnpm build       # nếu có thay đổi build output
```

Điền kết quả vào `dev-docs/rebase/$VERSION.md` (phần **Kết quả**):
...
| ----------- | -------------------------------------------- | ----------------------- | -------- | -------------- |
| v2026.5.5   | [rebase/v2026.5.5.md](rebase/v2026.5.5.md)   | `rebase-vs-v2026.5.5`   | —        | Chưa thực hiện |
| v2026.5.4   | [rebase/v2026.5.4.md](rebase/v2026.5.4.md)   | `rebase-vs-v2026.5.4`   | —        | Chưa thực hiện |
| v2026.4.20  | [rebase/v2026.4.20.md](rebase/v2026.4.20.md) | `rebase-vs-v2026.4.20`  | —        | Đang thực hiện |
